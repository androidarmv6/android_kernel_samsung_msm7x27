...
#include "Firmware_BENNETT_R08_SW11.c"
#include "Firmware_BENNETT_R06_SW21.c"
#include "Firmware_BENNETT_R09_SW23.c"
#include "Firmware_BENNETT_R40_SW32.c"
...
static void mcsdl_set_ready(void)
{
	...
	mcsdl_delay(MCSDL_DELAY_30MS); 						// Delay '30 msec'
...
static void mcsdl_select_isp_mode(UINT8 ucMode)
{
	...
	MCSDL_RESETB_SET_HIGH();	mcsdl_delay(MCSDL_DELAY_3US);			// High
	...
	//---------------------------------------------------
	 ...
	 if( ucMode == ISP_MODE_ERASE_FLASH   ){
		...
		for(i=0; i<6; i++){
				 ...
				 if( i==3 ) mcsdl_delay(MCSDL_DELAY_25MS);
			else if( i==4 ) mcsdl_delay(MCSDL_DELAY_150US);

			MCSDL_GPIO_SCL_SET_HIGH();	mcsdl_delay(MCSDL_DELAY_5US);
			MCSDL_GPIO_SCL_SET_LOW();	mcsdl_delay(MCSDL_DELAY_5US);
		}
	}
}
...
static void mcsdl_scl_toggle_twice(void)
{
	...
	MCSDL_GPIO_SCL_SET_HIGH();	mcsdl_delay(MCSDL_DELAY_15US);
	MCSDL_GPIO_SCL_SET_LOW();	mcsdl_delay(MCSDL_DELAY_15US);
