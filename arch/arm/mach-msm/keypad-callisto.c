...
static unsigned short keypad_keymap_surf[ARRAY_SIZE(keypad_col_gpios) *
...
#if (CONFIG_BOARD_REVISION < 0x03)
	[KEYMAP_INDEX(2, 5)] = KEY_MENU,	// LCD 패널에 위치.
#else
	[KEYMAP_INDEX(2, 6)] = KEY_MENU,	// LCD 패널에 위치.
#endif
	...
	[KEYMAP_INDEX(3, 5)] = KEY_HOME,	// LCD 패널에 위치.
	...
	[KEYMAP_INDEX(4, 5)] = KEY_END, // HOLD Btn. Power on/off 겸용.
...
#if (CONFIG_BOARD_REVISION < 0x03)
	[KEYMAP_INDEX(7, 5)] = KEY_BACK,	// LCD 패널에 위치. 
#else
	[KEYMAP_INDEX(7, 6)] = KEY_BACK,	// LCD 패널에 위치. 
#endif
	// END Callisto KeyMap. 
